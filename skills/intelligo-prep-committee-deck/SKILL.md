...
## Resolving the subject / project in Intelligo
...
- **Preserve severity exactly as Intelligo assigned it.** Don't reclassify yellow as red, or vice versa.
- **Intelligo doesn't make investment recommendations** — don't fabricate or restate one.
...
- **Only describe what Intelligo returned.** Gaps get marked as "not checked" or "no findings" — no speculation.
...
- `references/profile-resolution.md` — shared logic for resolving the user's reference to a single Intelligo profile (used by all Intelligo skills)
