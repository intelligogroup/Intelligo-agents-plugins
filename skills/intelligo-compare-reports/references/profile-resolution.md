...
- Did a previous turn in this conversation resolve a profile? Look for: a prior call to `get_profiles` followed by user selection, a prior call to `get_profile` that returned data, or an explicit profile_id surfaced earlier.
...
**Call `get_profiles` and `get_projects` in parallel** with the user's query.
